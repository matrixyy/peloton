...
bool UpdateExecutor::DInit() {
  ...
  // For queue
  //  const planner::UpdatePlan *node =
  //      reinterpret_cast<const planner::UpdatePlan *>(GetRawNode());
  //  const planner::ProjectInfo *proj_info = node->GetProjectInfo();
  //  target_table_ = node->GetTable();
  //  const planner::ProjectInfo new_info = new planner::ProjectInfo(proj_info);
  //  project_info_.reset(new planner::ProjectInfo(new_info));
  //  assert(&new_info);
